...
<article class="flex flex-col shadow my-4">
                ...
                <div class="bg-white flex flex-col justify-start p-6">
                    ...
                    <p href="/article/{{$post->slug}}" class="text-sm pb-3">
                        By <a href="/article/{{$post->slug}}" class="font-semibold hover:text-gray-800">{{$post->user->name}}</a>,
                        Published on {{$post->getFormattedDate()}}
                    </p>
                    <a href="/article/{{$post->slug}}" class="pb-6">
                        {{$post->shortBody()}}
                    </a>
                    <a href="/article/{{$post->slug}}" class="uppercase text-gray-800 hover:text-black">Continue Reading <i class="fas fa-arrow-right"></i></a>
                </div>
            </article>
